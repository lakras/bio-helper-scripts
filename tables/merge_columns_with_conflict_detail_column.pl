...
my $table = $ARGV[0];
my $title_of_column_with_merge_details = $ARGV[1]; # not to merge
my @titles_of_columns_to_merge = @ARGV[2..$#ARGV];
...
my $column_with_merge_details = -1;
	...
	if($line =~ /\S/) # if row not empty
	{
		...
		if($first_line) # column titles
		{
			...
			foreach my $column_title(@items_in_line)
			{
				...
				{
					$column_with_merge_details = $column;
				}
				$column++;
			}
			...
			foreach my $column_title(keys %column_title_to_column)
			{
				...
				elsif($column_with_merge_details == -1)
				{
		...
		{
			# retrieves details to print in case of merge conflict
			my $merge_details = $items_in_line[$column_with_merge_details];
				...
				or $all_numeric and !$values_all_close_enough)
			{
				...
				print STDERR " in ".$merge_details if $merge_details;
				print STDERR ":";
				foreach my $value(keys %value_to_merge_to_source)
				{
